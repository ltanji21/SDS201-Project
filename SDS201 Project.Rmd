...
Austin_Animals3 <- Austin_Animals2 %>%
  group_by(Homed) %>%
  summarise(Blackish = sum(Black == "1", Homed == "1"),
            notBlack = sum(Black == "0", Homed == "1"),
            Male = sum(Sex == "0", Homed == "1"),
            Female = sum(Sex == "1", Homed == "1"),
            unsexed = sum(Fixed == "1", Homed == "1"),
            sexed = sum(Fixed == "0", Homed == "1"),
            Canine = sum(Animal.Type == "Dog"),
            Feline = sum(Animal.Type == "Cat"))
...
ggplot(Austin_Animals4, aes(x = variable, y = value, fill = Adopted)) + geom_bar(stat ="identity")
