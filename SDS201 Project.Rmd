...
*Str*
The Str describes each of the twelve variables that we have in the dataframe “Adopted”. `Animal.ID` has 73442 levels which correspond to an adopted animal. The variable with the most levels. Interestingly, it has more entries than the dimension of the data frame itself. We are unsure about why this may be since each animal should only have one unique identifier. 
...
Again, Date.of.Birth also only has 5987 levels since some animals may have been born on the same day and year. 
